...
for iii in scc:
    ...
    for i in range(0, len(pfad_gpm)):
        ...
        #phaseNearSurface
        dpr_phase=np.array(gpmdpr[sc]['SLV']['phaseNearSurface'], dtype=float)
        #typePrecip
        dpr_typ = np.array(gpmdpr[sc]['CSF']['typePrecip'], dtype=float)
        ########################################################################
        gpm_time = gpmdpr[sc]['ScanTime']

        print dpr_pp.shape, dpr_typ.shape
        ...
        try:
            ...
            try:
                ...
                w2h('/automount/ags/velibor/gpmdata/dumpdata/SKILL_'+sc+'/'+sc+ str(gpm_zeit),
                    gpm_x, gpm_y, ggg_pp, ggg_bbh, ggg_bbw, ggg_phase, ggg_typ)
                print '/automount/ags/velibor/gpmdata/dumpdata/SKILL_'+sc+ str(gpm_zeit)


                del(gprof_lat,
                        gprof_lon, gprof_pp, res_bin, rrr, ggg_pp, ggg_bbh, ggg_bbw,
                    ...
                    dpr_bbw, dpr_phase, dpr_typ,dpr_pp_b,dpr_bbh_b,dpr_bbw_b,dpr_phase_b,dpr_typ_b)
