...
pfad2 = ('/automount/radar/dwd/'+ r_pro +'/'+str(year)+'/'+str(year)+'-'+
        str(m)+'/'+ str(year)+'-'+str(m)+'-'+str(d)+'/raa01-'+r_pro+'_10000-'+
        str(ye)+str(m)+ str(d)+str(ht)+str(mt)+'-dwd---bin.gz')

pfad_radolan2 = pfad[:-3]

try:
    rw_filename2 = wradlib.util.get_wradlib_data_file(pfad2)
except EnvironmentError:
    rw_filename2 = wradlib.util.get_wradlib_data_file(pfad_radolan2)
...
rwdata3 = np.nanmean(rwdata, rwdata2)
...
pm1 = plt.pcolormesh(x, y, rwdata3, cmap=my_cmap, vmin=0.01, vmax=50, zorder=2)
