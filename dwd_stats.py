...
fig = plt.figure(figsize=(16,10))
fig.autofmt_xdate()
plt.errorbar(time,A, B, fmt='o', zorder=1, color='grey')
#plt.scatter(range(0,len(A)),A,linewidths=0.001, vmin=T, vmax=1000, cmap=get_my_cmap(), zorder=2)
plt.ylim(-1,1)
plt.xlim(0,len(A))
plt.grid()
plt.ylabel('Correlation',fontsize=ff)
...
aa.plot(fmt='ko', yerr=bb, label='Correlation Coefficient')

pd.rolling_mean(aa,80, center=False,
                min_periods=2).plot(linewidth=3, color='red',
                                    label='Rolling Median\nW: 40 Overpasses')

plt.ylim(-1,1)
plt.xticks(rotation=45, fontsize=ff)
plt.yticks(fontsize=ff)
plt.grid(color='black')
plt.ylabel('Correlation Coefficient', fontsize=ff)
plt.title('Overpass Statistics between DPR and RADOLAN \n'
          '2014-03-18 - 2017-02-20 \nThreshold: N = '+str(T),fontsize=ff)
plt.legend(loc='lower right', fontsize=ff, numpoints=1)
plt.show()